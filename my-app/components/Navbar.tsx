  ...
  return (
    <nav className="bg-blue-700 dark:bg-blue-900 text-white px-6 py-4 shadow-md">
      <div className="flex items-center justify-center space-x-6 relative">
        ...
        <div className="relative">
          ...
          {showSearch && (
            <input
              ...
              className="absolute top-8 left-0 w-48 px-3 py-1 rounded text-black dark:text-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 transition"
            />
          )}
        </div>
        ...
        {/* Account / Login */}
        <Link
          href={currentUser ? (currentUser.isAdmin ? `/admin/${currentUser.id}` : `/user/${currentUser.id}`) : "/login"}
