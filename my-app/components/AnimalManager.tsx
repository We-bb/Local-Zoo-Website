...
export default function AnimalManager() {
  ...
  const handleAddAnimal = async (e: React.FormEvent) => {
    e.preventDefault();
    const payload = { ...formData, age: formData.age ? Number(formData.age) : undefined, birthDate: formData.birthDate || undefined };
    const res = await fetch("/api/animals", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    if (res.ok) { setFormData({ name: "", species: "", age: "", description: "", image: "", location: "", birthDate: "", naturalHabitat: "" }); setShowForm(false); fetchAnimals(); }
  ...
  const handleEdit = (animal: Animal) => { setEditingId(animal.id); setEditData({ ...animal }); };
  const handleSaveEdit = async (animalId: string) => {
    const payload = { ...editData, age: editData.age !== undefined ? Number(editData.age) : undefined, birthDate: editData.birthDate || undefined };
    const res = await fetch(`/api/animals/${animalId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    if (res.ok) { setEditingId(null); fetchAnimals(); }
    else { const err = await res.json(); alert("Error: " + err.error); }
  };
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded transition-colors">
          <FaPlus /> Add Animal
        </button>
      </div>
      ...
      {/* Add Form */}
      {showForm && (
        <form onSubmit={handleAddAnimal} className="bg-gray-200 dark:bg-gray-700 p-4 rounded mb-6 space-y-2">
          ...
          <li key={animal.id} className="flex flex-col md:flex-row justify-between items-start md:items-center bg-gray-100 dark:bg-gray-800 p-2 rounded">
            ...
            {editingId === animal.id ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 w-full mt-2">
                ...
                <input value={editData.location || ""} onChange={e => setEditData({ ...editData, location: e.target.value })} className="p-1 rounded border" placeholder="Location" />
                <input value={editData.naturalHabitat || ""} onChange={e => setEditData({ ...editData, naturalHabitat: e.target.value })} className="p-1 rounded border" placeholder="Natural Habitat" />
                ...
                <input value={editData.image || ""} onChange={e => setEditData({ ...editData, image: e.target.value })} className="p-1 rounded border col-span-2" placeholder="Image URL" />
                <input value={editData.description || ""} onChange={e => setEditData({ ...editData, description: e.target.value })} className="p-1 rounded border col-span-2" placeholder="Description" />
                <div className="flex gap-2 mt-2 col-span-2 justify-end">
                  <button onClick={() => handleSaveEdit(animal.id)} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded flex items-center gap-1"><FaSave /> Save</button>
                  <button onClick={() => setEditingId(null)} className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded flex items-center gap-1"><FaTimes /> Cancel</button>
                </div>
              </div>
            ) : (
